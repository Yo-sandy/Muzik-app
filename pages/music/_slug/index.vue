  ...
  <div class="slug-section">
    <div class="content-page">
      <div class="iq-card p-md-3">
        <div class="col-lg-12">
          <div class="row" v-if="music != undefined && Object.keys(music).length > 0">
            <h2 class="text-center trending-song">{{music.name}}</h2>
            <hr>
            <div class="col-lg-6 songs-item"
                 v-for="(single_song, index) in music.song_data"
                 :key="single_song.id"
            >
              <div class="player2">
...
export default {
  ...
  mounted() {
    this.loadData();
    this.$store.dispatch('musics/getMusics')
    if(process.client){
  ...
  methods:{
    loadData(){
      this.$store.dispatch('musics/getMusics').then(res=>{
        this.music = this.$store.getters['musics/getMusicByID'](this.$route.params.slug)
      })
    },

    playSong(single_song, index){
      this.$store.dispatch('player/setPlayerSong', {song:single_song, index:index});
      this.$store.dispatch('player/setSong', this.music.song_data);
    },
  },
}
</script>
