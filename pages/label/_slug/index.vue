  ...
  <div class="slug-section">
    <div class="content-page">
      <div class="iq-card p-md-3">
        <div class="col-lg-12">
          <div class="row" v-if="label != undefined && Object.keys(label).length > 0">
            <h2 class="text-center trending-song">{{label.name}}</h2>
            <hr>
            <div class="col-lg-6 songs-item"
                 v-for="(single_song, index) in label.song_data"
                 :key="single_song.id"
            >
              <div class="player2" >
...
export default {
  ...
  methods:{
    loadData(){
      this.$store.dispatch('labels/getLabels').then(res=>{
        this.label = this.$store.getters['labels/getLabelByID'](this.$route.params.slug)
      })
    },

    playSong(single_song, index){
      this.$store.dispatch('player/setPlayerSong', {song:single_song, index:index});
      this.$store.dispatch('player/setSong', this.artist.song_data);
    },
  },
}
</script>
